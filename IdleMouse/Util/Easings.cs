...
public static class Easings
{
    ...
    {
        return FMath.Sin((p - 1) * FMath.PI_2) + 1;
    }
    ...
    {
        return 0.5f * (1 - FMath.Cos(p * FMath.PI));
    }
    ...
    {
        return FMath.Sqrt((2 - p) * p);
    }

    /// <summary>	
    /// Modeled after the piecewise circular function
    /// y = (1/2)(1 - FMath.Sqrt(1 - 4x^2))           ; [0, 0.5)
    /// y = (1/2)(FMath.Sqrt(-(2x - 3)*(2x - 1)) + 1) ; [0.5, 1]
    /// </summary>
    public static float CircularEaseInOut(float p)
    {
        if (p < 0.5f)
        {
            return 0.5f * (1 - FMath.Sqrt(1 - 4 * (p * p)));
        }
        else
        {
            return 0.5f * (FMath.Sqrt(-((2 * p) - 3) * ((2 * p) - 1)) + 1);
        }
    }
    ...
    {
        return (p == 0.0f) ? p : FMath.Pow(2, 10 * (p - 1));
    }
    ...
    {
        return (p == 1.0f) ? p : 1 - FMath.Pow(2, -10 * p);
    }
    ...
    public static float ExponentialEaseInOut(float p)
    {
        ...
        if (p < 0.5f)
        {
            return 0.5f * FMath.Pow(2, (20 * p) - 10);
        }
        else
        {
            return -0.5f * FMath.Pow(2, (-20 * p) + 10) + 1;
        }
    }

    /// <summary>
    /// Modeled after the damped sine wave y = sin(13pi/2*x)*FMath.Pow(2, 10 * (x - 1))
    /// </summary>
    public static float ElasticEaseIn(float p)
    {
        return FMath.Sin(13 * FMath.PI_2 * p) * FMath.Pow(2, 10 * (p - 1));
    }

    /// <summary>
    /// Modeled after the damped sine wave y = sin(-13pi/2*(x + 1))*FMath.Pow(2, -10x) + 1
    /// </summary>
    public static float ElasticEaseOut(float p)
    {
        return FMath.Sin(-13 * FMath.PI_2 * (p + 1)) * FMath.Pow(2, -10 * p) + 1;
    }

    /// <summary>
    /// Modeled after the piecewise exponentially-damped sine wave:
    /// y = (1/2)*sin(13pi/2*(2*x))*FMath.Pow(2, 10 * ((2*x) - 1))      ; [0,0.5)
    /// y = (1/2)*(sin(-13pi/2*((2x-1)+1))*FMath.Pow(2,-10(2*x-1)) + 2) ; [0.5, 1]
    /// </summary>
    public static float ElasticEaseInOut(float p)
    {
        if (p < 0.5f)
        {
            return 0.5f * FMath.Sin(13 * FMath.PI_2 * (2 * p)) * FMath.Pow(2, 10 * ((2 * p) - 1));
        }
        else
        {
            return 0.5f * (FMath.Sin(-13 * FMath.PI_2 * ((2 * p - 1) + 1)) * FMath.Pow(2, -10 * (2 * p - 1)) + 2);
        }
    }
    ...
    {
        return p * p * p - p * FMath.Sin(p * FMath.PI);
    }
    ...
    {
        float f = (1 - p);
        return 1 - (f * f * f - f * FMath.Sin(f * FMath.PI));
    }
    ...
    public static float BackEaseInOut(float p)
    {
        ...
        {
            float f = 2 * p;
            return 0.5f * (f * f * f - f * FMath.Sin(f * FMath.PI));
        }
        else
        {
            float f = (1 - (2 * p - 1));
            return 0.5f * (1 - (f * f * f - f * FMath.Sin(f * FMath.PI))) + 0.5f;
        }
    }
